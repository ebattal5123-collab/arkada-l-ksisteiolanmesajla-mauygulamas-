...
def init_db():
    ...
    for room_name in default_rooms:
        try:
            rooms_collection.insert_one({'name': room_name, 'created_at': datetime.now(), 'type': 'public'})
...
<body>
    ...
    <div class="profile-modal" id="profileModal">
        <div class="profile-box">
            ...
            <div class="profile-info">
                ...
                <div class="profile-field">
                    <label>NOT</label>
                    <div class="profile-field-value" style="color: #666; font-size: 12px;">
                        Bu ID sadece size özeldir. Arkadaş eklemek veya grup kurmak için diğer kullanıcılarla paylaşın.
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        function addRoomToList(roomName, isPrivate = false, isGroup = false) {
            ...
            if (isGroup) {
                displayName = roomName.split('_')[2]; // Grup adını al
            }
            
            roomItem.innerHTML = `<span class="room-icon">${icon}</span><span class="room-name">${displayName}</span>`;
            ...
            .then(messages => {
                ...
                if (messages.length === 0) {
                    ...
                    if (roomName.includes('_group_')) {
                        roomDisplayName = roomName.split('_')[2];
                    }
...
def login():
    try:
        ...
        logger.info(f'✅ Kullanıcı giriş yaptı: {username}, ID: {user["user_id"]}')
